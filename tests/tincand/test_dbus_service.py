...
Bead: tincan-f0e  (ref: review finding F1, tincan-mj3), tincan-efv3
...
Coverage:
  ...
  §6 _resolve_to_phone — send_target shortcut:
    - conv.send_target non-empty → returned directly, bypassing id-normalise fallback
    - conv.send_target empty, conv.id is phone-shaped → normalize_phone(conv.id) returned
    - conv.send_target empty, conv.id is name-keyed → falls through, returns 'to' unchanged
...
class TestResolveToPhoneSendTargetShortcut:
    """_resolve_to_phone returns conv.send_target when set; falls through when absent."""

    @staticmethod
    def _cs():
        """Mock ContactStore with no PBAP entries."""
        cs = MagicMock(name="ContactStore")
        cs.lookup_by_name.return_value = None
        return cs

    def test_send_target_present_returned_directly(self):
        # Name-keyed conv.id; send_target carries the phone — shortcut must fire
        convs = {
            "Alice": Conversation(id="Alice", display_name="Alice", send_target="5551234567"),
        }
        assert TincanService._resolve_to_phone("Alice", convs, self._cs()) == "5551234567"

    def test_send_target_present_not_same_as_conv_id(self):
        # Confirms the result is send_target, not conv.id
        convs = {
            "Alice": Conversation(id="Alice", display_name="Alice", send_target="5551234567"),
        }
        result = TincanService._resolve_to_phone("Alice", convs, self._cs())
        assert result != "Alice"

    def test_send_target_absent_phone_id_normalises_correctly(self):
        # conv.id is phone-shaped; send_target empty → falls through to normalize_phone(id)
        convs = {
            "5557654321": Conversation(id="5557654321", display_name="Bob", send_target=""),
        }
        assert TincanService._resolve_to_phone("Bob", convs, self._cs()) == "5557654321"

    def test_send_target_absent_name_id_falls_through_to_to(self):
        # conv.id is name-keyed; send_target empty → normalize_phone gives no digits → return 'to'
        convs = {
            "Alice": Conversation(id="Alice", display_name="Alice", send_target=""),
        }
        assert TincanService._resolve_to_phone("Alice", convs, self._cs()) == "Alice"
