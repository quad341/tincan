...
Coverage:
  ...
  §5 _norm_phone — canonical phone key derivation
    - '+18157916347' → '8157916347' (country-code strip, 11 digits → last 10)
    - '8157916347' → '8157916347' (exactly 10 digits, passthrough)
    - '+1 (815) 791-6347' → '8157916347' (strip formatting + country code)
    - 'Alice' → 'Alice' (display name, no digits ≥7, raw passthrough)
    - '1234567' → '1234567' (7-digit edge, ≤10 digits so no truncation)
    - '12345' → '12345' (sub-7 digit count, raw passthrough)
  §6 MapBackend.poll_inbox — Datetime/DateTime/Date or-chain fallback
    - 'Datetime' key → timestamp extracted correctly
    - 'DateTime' key → timestamp extracted via fallback
    - 'Date' key → timestamp extracted via second fallback
...
class TestMapBackendDatetimeKeyFallback:
    """poll_inbox reads the raw date via props['Datetime'] or ['DateTime'] or ['Date']."""
    ...
    def _poll_timestamp(self, **props):
        ...
        mock_access.GetMessage.return_value = None  # skip full-body fetch
    ...
    def test_datetime_camel_key_fallback_yields_correct_timestamp(self):
        # 'Datetime' absent — should fall back to 'DateTime'
    ...
    def test_date_key_second_fallback_yields_correct_timestamp(self):
        # Both 'Datetime' and 'DateTime' absent — should fall back to 'Date'
