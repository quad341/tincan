"""Tests: long unbroken words get &#8203; break hints in message bubbles (tincan-5hcyf).

Coverage:
  - _break_long_words inserts &#8203; every 30 chars in a long plain token.
  - _break_long_words leaves short words unchanged.
  - _break_long_words resets count at whitespace boundaries.
  - _break_long_words does not insert &#8203; inside HTML tag attributes (href=).
  - _linkify output contains &#8203; for a body with a 60-char unbroken word.
  - _linkify short text is unchanged (no spurious &#8203;).
  - MessageBubble body label text contains &#8203; for a long-word message.
"""
...
from tincan_gui.text_render import _break_long_words, _linkify
...
class TestBreakLongWords:
    """_break_long_words inserts &#8203; every 30 chars in long continuous sequences."""
    ...
    def test_whitespace_resets_counter(self):
        # 29 a's, space, 29 b's — neither run reaches 30, so no &#8203;
        text = "a" * 29 + " " + "b" * 29
    ...
    def test_inserts_wbr_in_link_display_text(self):
        ...
        html = f'<a href="{long_url}">{long_url}</a>'
        result = _break_long_words(html)
        # Display text (between > and <) should have &#8203; inserted
...
# ---------------------------------------------------------------------------
# §2 _linkify — long words in output get &#8203;
# ---------------------------------------------------------------------------

class TestLinkifyBreaksLongWords:
    """_linkify must include &#8203; hints for long unbroken words."""
...
class TestMessageBubbleLongWordWrap:
    """MessageBubble body label must contain &#8203; for a body with a 60-char word."""
