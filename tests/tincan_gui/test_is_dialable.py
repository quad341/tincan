...
§1 _is_dialable() — pure function
   ...
   - 6-digit short code IS dialable (threshold is ≥4 to support short codes)
   ...
   - 3-digit number is NOT dialable (below 4-digit threshold)
"""
...
class TestIsDialable:
    """≥4 digits → True (phone numbers + short codes); <4 digits or no digits → False."""
    ...
    def test_six_digit_short_code_is_dialable(self):
        # 6-digit short codes are dialable (threshold is ≥4)
        assert _is_dialable("555000") is True
    ...
    def test_name_with_two_digits_is_not_dialable(self):
        # "Bob 12" has only 2 digits — below 4-digit threshold
    ...
    def test_exactly_four_digits_boundary(self):
        # Boundary: exactly 4 digits (short code lower bound)
        assert _is_dialable("1234") is True

    def test_three_digits_boundary_is_not_dialable(self):
        # One below threshold: 3 digits
        assert _is_dialable("123") is False
