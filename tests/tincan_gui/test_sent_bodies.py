...
class TestSentBodiesPopulation:
    ...
    def test_successful_send_adds_body_to_sent_bodies(self, qtbot):
        ...
        window._on_send_accepted(phone, "Hello there", "msg-id-1")

        assert "Hello there" in window._sent_bodies.get(phone, set())

    def test_successful_send_uses_phone_as_key(self, qtbot):
        phone = "+15550002222"
        window = _make_window_with_phone(qtbot, phone)

        window._on_send_accepted(phone, "Keyed message", "msg-id-1")

        assert phone in window._sent_bodies

    def test_multiple_sends_accumulate_in_same_key(self, qtbot):
        phone = "+15550001111"
        window = _make_window_with_phone(qtbot, phone)

        window._on_send_accepted(phone, "First", "msg-id-1")
        window._on_send_accepted(phone, "Second", "msg-id-2")
...
class TestOnMessageReceivedSuppression:
    ...
    def test_outbound_body_in_sent_bodies_not_appended(self, qtbot):
        phone = "+15550001111"
        window = _make_window_with_phone(qtbot, phone)
        # Use _on_send_accepted directly to populate _sent_bodies (requires D-Bus in real flow)
        window._on_send_accepted(phone, "Suppress me", "msg-id-1")
...
class TestSentBodiesDisconnect:
    ...
    def test_sent_bodies_cleared_on_disconnect(self, qtbot):
        phone = "+15550001111"
        window = _make_window_with_phone(qtbot, phone)
        window._on_send_accepted(phone, "Will be cleared", "msg-id-1")
