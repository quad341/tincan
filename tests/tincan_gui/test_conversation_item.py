...
Beads: tincan-a3n  (tincan-33k preview text, tincan-yq1 unread badge)
       tincan-fii9.2 (§7 dark-mode selection stylesheet scoping)
...
Coverage:
  ...
  - §7 Dark-mode selection: styleSheet() uses scoped ConversationItem { } + QLabel reset
"""
...
# ---------------------------------------------------------------------------
# §7 Dark-mode selection stylesheet — scoped class selector + QLabel reset
# ---------------------------------------------------------------------------

class TestDarkModeSelectionStylesheet:
    """Selected dark-mode ConversationItem must scope its background to the
    'ConversationItem' class selector and explicitly reset child QLabel
    backgrounds to transparent, preventing Qt from propagating the selection
    colour to each QLabel as a separate blue rectangle.

    Regression guard for tincan-fii9.1.
    """

    def test_selected_stylesheet_contains_transparent_qlabel(self, qtbot):
        with patch("tincan_gui.conversation_list.is_dark_theme", return_value=True):
            item = ConversationItem(_make_data())
        qtbot.addWidget(item)
        item.set_selected(True)
        sheet = item.styleSheet()
        assert "QLabel { background: transparent; }" in sheet, (
            f"Expected 'QLabel {{ background: transparent; }}' in dark-mode selected "
            f"stylesheet to prevent child-widget bleed; got: {sheet!r}"
        )

    def test_selected_stylesheet_uses_scoped_class_selector(self, qtbot):
        with patch("tincan_gui.conversation_list.is_dark_theme", return_value=True):
            item = ConversationItem(_make_data())
        qtbot.addWidget(item)
        item.set_selected(True)
        sheet = item.styleSheet()
        assert "ConversationItem {" in sheet or "ConversationItem{" in sheet, (
            f"Expected scoped 'ConversationItem {{ ... }}' selector in dark-mode selected "
            f"stylesheet; bare background-color propagates to child QLabels; got: {sheet!r}"
        )

    def test_deselected_stylesheet_does_not_contain_dark_selected_bg(self, qtbot):
        with patch("tincan_gui.conversation_list.is_dark_theme", return_value=True):
            item = ConversationItem(_make_data())
        qtbot.addWidget(item)
        item.set_selected(True)
        ...
        sheet = item.styleSheet()
        dark_bg = ConversationItem._SELECTED_BG_DARK
        assert dark_bg not in sheet, (
            f"Dark-mode selected background '{dark_bg}' must be absent after "
            f"deselection; got: {sheet!r}"
        )
