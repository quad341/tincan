...
Note: §1 (successful send via send_message) is superseded by
test_send_async.py::TestOnSendUsesAsync which tests the async send path.

Coverage:
  §2 _on_send — failed send (no D-Bus session bus → message_send_failed)
