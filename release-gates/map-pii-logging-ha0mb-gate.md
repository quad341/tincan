...
| 2 | Acceptance criteria met | **PASS** | Per-poll `_log.warning` blocks dumping `Sender number + Subject` removed from `poll_inbox` (inbox and sent). `_sent_folder_warned` flag added: logs sent-folder notice once per session (INFO level), resets on `connect()` (new MAP session). Downgraded repeated empty-folder notices to DEBUG. |
